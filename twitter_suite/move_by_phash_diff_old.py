...
def move_similar_images(source_root, destination_root, csv_file, phash_diff_threshold):
    ...
    with mp.Pool(mp.cpu_count()) as pool:
        phash_diff_partial = partial(
            phash_diff, phash_dict=phash_dict, phash_diff_threshold=phash_diff_threshold
        )
        with tqdm(
            total=len(combinations), desc="Calculating phash differences"
        ) as progress_bar:
