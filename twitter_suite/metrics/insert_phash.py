...
def insert_phash(source_root: str, csv_file: Optional[str] = None) -> None:
    ...
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        phash_values = list(tqdm(executor.map(calculate_phash, [file_dict[file] for file in files_to_update]), total=len(files_to_update), desc="Calculating phash values"))

    updated_phash_values = dict(zip(files_to_update, phash_values))

    # Update the phash values in the DataFrame
    df.loc[df["filename"].isin(files_to_update), "phash"] = df["filename"].map(updated_phash_values)
